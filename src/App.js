...
class App extends React.Component {
  ...
  render() {
    const challengeFilter = filter => challenge =>
      challenge.toLowerCase().indexOf(filter.toLowerCase()) !== -1;
      ...
      <HashRouter>
        <div style={{ padding: "5% 5%" }}>
            ...
            >
              <div>
                <TextFilter
                  ...
                  placeholder="Search for a Challenge (min. 3 characters)"
                  minLength={minLengthForChallengeFilter}
                />
                <List
          ...
          <Button variant="outlined" color="primary">
            <Link
              href={"http://simplemethod.com/GNAR.pdf"}
              target="_blank"
              rel="noopener"
            >
          ...
          <Button variant="outlined" color="primary">
            <Link
              href={
                "https://www.vail.com/the-mountain/about-the-mountain/trail-map.aspx"
              }
              target="_blank"
              rel="noopener"
            >
        ...
        {items.map(item => (
          <TableRow key={item}>
            <TableCell>
              <GreenButton
                variant="contained"
                onClick={e => challengeCompleted(item)}
              >
                Complete
              </GreenButton>
            </TableCell>
            <TableCell>{item}</TableCell>
            <TableCell>{challenges.get(item).points}</TableCell>
            <TableCell>{challenges.get(item).description}</TableCell>
          </TableRow>
        ))}
      </TableBody>
    </Table>
...
function CollapsibleChallengesCompleted({
  challenges,
  completed,
  challengeDecrement
}) {
  if (
    Array.from(completed) === "undefined" ||
    Array.from(completed).length === 0
  ) {
    return (
      <div style={{ padding: "2em .1em" }}>
        <Card
          style={{ minWidth: 275, padding: "15px", border: "1px solid #ccc" }}
        >
          <Collapsible
            transitionTime={300}
          ...
          >
            <h5>
              You haven't completed any challenges yet! Get out there for heck's
              sake!
            </h5>
          </Collapsible>
        </Card>
            ...
            <TableBody>
              {Array.from(completed).map(value => (
                <TableRow>
                  ...
                  <TableCell>{challenges.get(value[0]).points}</TableCell>
                  <TableCell>{challenges.get(value[0]).description}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
...
function Scoreboard({ challenges, completed }) {
  var score = 0;
  completed.forEach(
    (timesCompleted, challengeId) =>
      (score += challenges.get(challengeId).points * timesCompleted)
  );

  return <h2>GNAR Value: {score}</h2>;
}

function CollapsibleCategoryCollection({
  challenges,
  categories,
  challengeCompleted
}) {
